...
| `Compares` | Boolean combinations of comparisons. For one operand pair: the five relations {EQ, (LTu,LTs), (LTu,GTs), (GTu,LTs), (GTu,GTs)}, each predicate a set, `& \| ^ ~` set operations, emitted when the set is one predicate, true or false (at W = 1 only three relations occur). For one operand against constants: exact unsigned interval sets (signed comparisons split at the sign boundary, at most 8 intervals), emitted as a comparison or the wrapped range check `x − lo <=u hi − lo`. Over a few atoms (the *order* reading, `pass/order.rs`): a combination of comparisons between up to 5 terms of one width, a term being an atom or a `select` between terms on such a condition (a minimum or maximum written out), is read in every total preorder the atoms can stand in (constants in their order, facts' disjoint ranges, `x & y <=u x`, `x <=u x \| y`, shifts, remainders and quotients by a nonzero divisor below their dividend, `x >>u s`, `udiv(x, d)` and `x >>s s` monotone), and emitted when it is one value, one comparison of two atoms, a part of itself, one atom, or a minimum or maximum of two. Checked exhaustively at W ≤ 6. `e == c` is also decided by the residues of `e` (`pass/residue.rs`: the low `k ≤ 4` bits of `+ − * neg ~ & \| ^` and constant shifts are a function of the leaves' low bits, tabled over up to 3 leaves): false when no residue matches `c`. |
...
pub mod mba {
    ...
    pub struct NormalFormSolver;  // the default: native normal forms, linear, semi-linear, polynomial MBA, atoms
    ...
    pub struct MbaTrust { pub backend_certificates: bool /* default false */, pub sampled: bool /* default false */ }
}
...
- **Gate.** An answer must have the input's variables and width, agree with the input at 64
  ...
  `trust.backend_certificates` (not the default), or agreement at the sampled points only if
  ...
  caught only when bitwright's own evidence applies, so it is off by default; hosts that trust
  their backend set `backend_certificates: true`. The gate's own evaluations (sampling and certificates, in
...
- **The normal-form solver** (`NormalFormSolver`, id `bitwright.nf.v3;…` with its options).
  ...
  - It is the default solver since 0.11, with backend certificates not trusted, as
    `docs/proposals/mba-defaults.md` proposed with a corpus diff (`bitwright-bench
    --corpus-diff`): much smaller results on MBA, at a time cost that is largest on code that
    is not obfuscated. `SignatureSolver` stays available for hosts that want linear MBA only,
    fast.
